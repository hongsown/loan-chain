...
func (msg *MsgRequestLoan) ValidateBasic() error {
	...
	if err != nil {
		return errorsmod.Wrap(sdkerrors.ErrInvalidRequest, "deadline is not a valid int64")
	}
	if deadline <= 0 {
		return errorsmod.Wrap(sdkerrors.ErrInvalidRequest, "deadline must be positive")
	}
